document.addEventListener("DOMContentLoaded", function () {
    ...
    $('#id_entrance_edge').selectize({
        create: false,
        options: [],
    });

    $('#id_custom_destination').selectize({
        create: false,
        sortField: 'title',
        sortField: 'title',
        valueField: 'id',
    ...
    document.getElementById('add-bill').addEventListener('click', e => {
        ...
        let billingTrackingNumber = document.getElementById('billing_tracking_number');
        let row = `<tr>
                        <td><input class="form-control border-0" value="${billingTrackingNumber.value}" type="text" name="${prefix}-${billingFormCount}-tracking_code" id="id_${prefix}-${billingFormCount}-tracking_code" style="background:rgba(0,0,0,0);"></td>
                        ...
                        <td><input type="checkbox" class="form-check-input row-checkbox" name="${prefix}-${billingFormCount}-DELETE" id="id_${prefix}-${billingFormCount}-DELETE"></td>
                    </tr>`;
        try {
    ...
    document.getElementById('whitout-transfer-currency-form').addEventListener('submit', e => {
        let rows = Array.from(wareTableBody.children);
        let wareFlag = false
        rows.forEach(row => {
            let checkbox = row.querySelector('.ware-checkbox');
